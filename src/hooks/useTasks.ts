...
export function useTasks(leadId?: string) {
  ...
  const addTask = useMutation({
    mutationFn: async (task: { lead_id: string; title: string; notes?: string; due_at?: string }) => {
  ...
  const markDone = useMutation({
    mutationFn: async ({ leadId, actionType, metadata }: { leadId: string; actionType: string; metadata?: any }) => {
      // Log action
      await supabase.from("action_log").insert({
        user_id: user!.id,
        lead_id: leadId,
  ...
  return {
    ...
    markDone: (leadId: string, actionType: string, metadata?: any) => markDone.mutateAsync({ leadId, actionType, metadata }),
