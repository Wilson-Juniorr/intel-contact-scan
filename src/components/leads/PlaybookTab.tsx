    ...
    <div className="space-y-4">
        ...
        <Card className="border-primary/20">
          <CardContent className="p-3 space-y-2">
            ...
            <p className="text-[10px] text-muted-foreground italic">
              ⚠️ {playbook.priorityTrigger} → Urgente após {playbook.daysUrgent}d, Crítico após{" "}
              {playbook.daysCritical}d
            </p>
          </CardContent>
        </Card>
      )}
        ...
        <div className="space-y-1.5">
          ...
          {playbook.tasks.map((pt, i) => {
            ...
            return (
              <div
                key={i}
                className="flex items-center gap-2 p-2 rounded-md border border-dashed border-border text-xs"
              >
      ...
      <div className="space-y-1.5">
        ...
        {generatingAiTasks && (
          <p className="text-[10px] text-muted-foreground animate-pulse">
            Analisando conversas, áudios, imagens e PDFs...
          </p>
        )}
        {aiTasks.map((at, i) => {
          ...
          return (
            <div
              key={i}
              className="p-2 rounded-md border border-primary/20 bg-primary/5 text-xs space-y-1"
            >
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  disabled={alreadyCreated}
                  onClick={() => handleAddTask(at.title)}
                >
                  {alreadyCreated ? (
                    <Check className="h-3 w-3" />
                  ) : (
        ...
        {aiTasks.length === 0 && !generatingAiTasks && (
          <p className="text-[10px] text-muted-foreground">
            Clique em "Analisar conversa" para gerar tarefas baseadas no histórico
          </p>
        )}
      </div>
      ...
      <div className="space-y-1.5">
        ...
        {openTasks.map((task: any) => (
          <div
            key={task.id}
            className="flex items-center gap-2 p-2 rounded-md border border-border bg-card text-xs group"
          >
            <button
          ...
          <details className="text-xs">
            ...
            <div className="space-y-1 mt-1">
              {doneTasks.map((task: any) => (
                <div
                  key={task.id}
                  className="flex items-center gap-2 p-1.5 text-muted-foreground line-through"
                >
      ...
      <div className="space-y-2">
        ...
        >
          ...
          {suggestedMsgs.length > 0
            ? "Regenerar follow-up estratégico"
            : "Gerar follow-up estratégico"}
        </Button>
          ...
          <div className="rounded-lg border border-border bg-muted/20 p-2.5 space-y-2">
            ...
            {timeline && (
              <div className="grid grid-cols-3 gap-x-2 gap-y-0.5 text-[9px] text-muted-foreground">
                ...
                {timeline.avg_response_time_days !== null && (
                  <span>⏱️ Resp. média: {timeline.avg_response_time_days}d</span>
                )}
              </div>
            )}
            ...
            {silenceStage && (
              <div className="flex items-center gap-1.5 flex-wrap">
                ...
                <Badge variant="outline" className="text-[8px]">
                  {flowPattern === "super_short"
                    ? "Flow curto"
                    : flowPattern === "validate_tension_direct"
                      ? "Validar→CTA"
                      : "Flow padrão"}
                </Badge>
              </div>
            ...
            {behavior && behavior.confidence !== "low" && (
              <div className="text-[9px] text-muted-foreground flex gap-2 flex-wrap">
                ...
                {behavior.energy_level && (
                  <span>
                    ⚡{" "}
                    {
                      ({ high: "Alta", medium: "Média", low: "Baixa" } as any)[
                        behavior.energy_level
                      ]
                    }
                  </span>
            ...
            {/* Guardrails */}
            {guardrails &&
              (guardrails.must_confirm_network ||
                guardrails.avoid_discount_promises ||
                guardrails.competitor_mode) && (
                <div className="flex items-center gap-1.5 flex-wrap">
        ...
        {suggestedMsgs.length > 0 && (
          <div className="space-y-2">
            {suggestedMsgs.map((msg, idx) => (
              <div
                key={idx}
                className="bg-muted/30 rounded-lg p-2.5 text-xs border border-border space-y-1.5"
              >
                <div className="flex items-center gap-1.5">
                  <Badge variant="secondary" className="text-[9px] h-4 px-1.5">
                    {idx + 1}/{suggestedMsgs.length}
                  </Badge>
                </div>
                {editingIdx === idx ? (
                  <textarea
            ...
            <div className="flex gap-2">
              ...
              <Button
                size="sm"
                className="flex-1 text-xs gap-1 h-7 bg-secondary hover:bg-secondary/90 text-secondary-foreground"
                asChild
              >
                <a
                  href={buildWhatsAppUrl(lead.phone, suggestedMsgs[0])}
                  target="_blank"
      ...
      {playbook && templates.length > 0 && (
        <div className="space-y-1.5">
          ...
          {templates
            .filter((t) => playbook.recommendedTemplates.includes(t.id))
            .map((tmpl) => {
