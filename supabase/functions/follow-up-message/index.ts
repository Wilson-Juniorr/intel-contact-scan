...
serve(async (req) => {
  ...
  try {
    const { lead, idleHours, idleDays, userContext } = await req.json();
...
- Notas: ${lead.notes || "nenhuma"}${userContext ? `\n\nCONTEXTO DO VENDEDOR (use isso para personalizar a mensagem):\n${userContext}` : ""}`,
          },
        ],
      }),
    });
